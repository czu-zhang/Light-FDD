...
from ultralytics.nn.modules import (
    ...
    BasicStage,
    PatchMerging_FasterNet_ContextGuided, conv_bn_hswish, MobileNetV3_InvertedResidual,
)
...
from ultralytics.nn.modules.attention import RFCBAMConv
from ultralytics.nn.modules.SlimNeck import VoVDualCSP
        ...
        if not self.is_fused():
            for m in self.model.modules():
                ...
                if type(m) is PatchMerging_FasterNet:
                    ...
                    delattr(m, 'norm')  # remove BN
                    m.forward = m.fuseforward
                # if type(m) is PatchMerging_FasterNet_ContextGuided:
                #     m.reduction = fuse_conv_and_bn(m.reduction, m.norm)
                #     delattr(m, 'norm')  # remove BN
                #     m.forward = m.fuseforward
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            RepC3,
            PSA,
            SCDown, conv_bn_hswish, MobileNetV3_InvertedResidual,
            C2fCIB, BasicStage, PatchEmbed_FasterNet, PatchMerging_FasterNet, PatchMerging_FasterNet_ContextGuided,VoVDualCSP,
        }:
            c1, c2 = ch[f], args[0]
            ...
            args = [c1, c2, *args[1:]]
            if m in (BottleneckCSP, C1, C2, C2f, C2fAttn, C3, C3TR, C3Ghost, C3x, RepC3, C2fCIB, VoVDualCSP):
